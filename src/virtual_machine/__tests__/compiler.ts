...
test('compile #%plain-lambda', () => {
  ...
  `);
  expect(programState1.funcIdToCompiledProgram.map((program) => prettify_compiled_program(program)))
    .toMatchInlineSnapshot(`
    Array [
      Array [
        "GET_ENV",
  ...
  expect(
    compileAndPrettify(
      getOk(
        read(`(#%plain-lambda (x . y) (#%variable-reference x) (#%variable-reference y))`)
      ) as ExprOrDefineForm,
      programState2
    )
  ).toMatchInlineSnapshot(`
  ...
  `);
  expect(programState2.funcIdToCompiledProgram.map((program) => prettify_compiled_program(program)))
    .toMatchInlineSnapshot(`
    Array [
      Array [
        "GET_ENV",
