...
> = new Map([
  ...
  ['#%provide', noop_statement_transformer],
...
export function expand_in_expression_context(
  ...
  for (;;) {
    ...
    if (identifier_name === '#%app') {
      return err('no #%app syntax transformer bound in ' + print(expr));
    }
...
export const compile_module: CompileModule = (
  ...
  if (module_path_is_builtin) {
    ...
    const parent_module_result = evaluate_module(
      read_parent_module_result.v as TopLevelModuleForm,
      parent_module_precompiled_filename,
      host
    );
    if (isBadResult(parent_module_result)) {
  ...
  if (module_path_is_builtin) {
    ...
    const fep_module: ModuleFileParentForm = slist(
      [
        ssymbol('module'),
        ssymbol(module_info.name),
